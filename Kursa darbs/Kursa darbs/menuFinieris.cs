...
{
    public partial class menuFinieris : Form
    {
        public menuFinieris()
        {
            InitializeComponent();
        ...
        private void menuBalki_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd.Connection = con;
                cmd.CommandText = "SELECT ID, Warehouse_location, Stock_count FROM Stock WHERE Timber_type='Finieris'";
                ...
                while (reader.Read())
                {
                    ...
                    lvwFinierisList.Items.Add(itemRow);
                }
                con.Close();
            }
            catch (Exception ex)
